...
baltimore <- subset(NEI, NEI$fips == "24510", select = fips:year)

typeAggYear <- baltimore %>% 
      group_by(type, year) %>% 
...
# Generate graph
ggplot(data = typeAggYear, 
               aes(x = factor(year), y = Emissions/1000, fill = type)) + 
      geom_bar(stat = "identity") + 
      facet_grid(facets = . ~ type) +
      geom_label(aes(label = round(Emissions/1000,2), fill = type), color = "white", fontface = "bold") +
      labs(y = expression('Total PM'[2.5]*' emission in kilotons'),
           x = "Year",
           title = expression('Total PM'[2.5]*' emission in Baltimore by Type'))
