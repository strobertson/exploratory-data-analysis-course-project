...
# Create two subsets for Baltimore City and LA
baltimoreroad <- subset(NEI, NEI$fips == "24510" & type == "ON-ROAD" , select = fips:year)
laroad <- subset(NEI, NEI$fips == "06037" & type == "ON-ROAD" , select = fips:year)

# Create human readable county column
baltimoreroad$County <- "Baltimore City, MD"
laroad$County <- "Los Angeles, CA"

comproad <- rbind(baltimoreroad,laroad)

comproadAggYear <- comproad %>%
...
# Generate graph
ggplot(data = comproadAggYear, 
       aes(x = factor(year), y = Emissions/1000, fill = County)) +
      facet_grid(facets = County~., scales = "free") +
      geom_bar(stat = "identity") + 
      geom_label(aes(label = round(Emissions/1000,2), fill = County), color = "white", fontface = "bold") +
      labs(y = expression('Total PM'[2.5]*' emission in kilotons'),
           x = "Year",
           title = expression('Variation in Total PM'[2.5]*' emission in Baltimore, MD and Los Angeles, CA from On-Road Sources'))

# Save graph to png file
ggsave("plot6.png", device = "png")

# End of step message
message("Graph generated and saved to working directory as plot6.png")
